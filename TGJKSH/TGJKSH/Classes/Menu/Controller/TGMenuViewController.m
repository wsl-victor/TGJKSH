...
@interface TGMenuViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate>
...
{
    TGMenuDetailViewController *menuDeatailVc=[[TGMenuDetailViewController alloc] init];
    menuDeatailVc.dict=self.foodArr[indexPath.row];
    [self.navigationController pushViewController:menuDeatailVc animated:YES];
}
...
-(void)requestNetWorkFoodList:(NSString *)classid
{
    ...
    dict[@"rows"]=@"10";
    [AllReqestAPIManager getMenuListClass:dict success:^(NSDictionary *json) {
