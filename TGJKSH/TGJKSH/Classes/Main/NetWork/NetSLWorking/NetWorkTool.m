...
+(BOOL)requestBeforeCheckNetWorkWithFailureBlock:(failureBlocks)errorBlock{
    ...
    if(!isFi){//无网络
        ...
        [NetWorkStates startNetworkActivity];
    }
...
+(AFHTTPSessionManager *)getHttpSessionManager
{
   ...
   // manager.responseSerializer=[af];
    manager.responseSerializer.stringEncoding = NSUTF8StringEncoding;//默认 NSUTF8StringEncodin
//    AFSecurityPolicy *securityPolicy = [AFSecurityPolicy defaultPolicy];
//    securityPolicy.allowInvalidCertificates = YES;
//    manager.securityPolicy = securityPolicy;
//    manager.responseSerializer set 
...
//    [manager.requestSerializer didChangeValueForKey:@"timeoutInterval"];
  ...
  //  manager.securityPolicy=[AFSecurityPolicy policyWithPinningMode:AFSSLPinningModeNone];
    manager.requestSerializer=[AFHTTPRequestSerializer serializer]; //[AFJSONRequestSerializer serializer];
   // [manager.requestSerializer setValue:[NSString stringWithFormat:@"%@",kVersion] forHTTPHeaderField:KEY_NET_HEAD];
