...
- (E_EveryChapter *)openChapter{
   ...
   // NSString *chapter_num = [NSString stringWithFormat:@"Chapter%ld",_currentChapterIndex];
   // NSString *path1 = [[NSBundle mainBundle] pathForResource:chapter_num ofType:@"txt"];
  NSDictionary *dict=  self.allListDataArr[_currentChapterIndex];
    
    chapter.chapterContent = dict[@"message"];//[NSString stringWithContentsOfFile:path1 encoding:4 error:NULL];
...
- (E_EveryChapter *)nextChapter{
    ...
    if (_currentChapterIndex >= _totalChapter) {
        ...
        NSDictionary *dict=self.allListDataArr[_currentChapterIndex];
        chapter.chapterContent =dict[@"message"];// readTextData(_currentChapterIndex);
...
- (void)resetTotalString{
    ...
    for (int i = 1; i <  INT_MAX; i ++) {
        NSDictionary *dict=self.allListDataArr[i];
        NSString *str=dict[@"message"];
        if (str != nil) {
            
            NSUInteger location = _totalString.length;
            [_totalString appendString:str];
...
//    //NSString *path1 = [[NSBundle mainBundle] pathForResource:chapter_num ofType:@"txt"];
//    NSDictionary *dict=self.allListDataArr[index];
//    NSString *content =dict[@"message"];//[NSString stringWithContentsOfFile:path1 encoding:4 error:NULL];
//    return content;
//}
