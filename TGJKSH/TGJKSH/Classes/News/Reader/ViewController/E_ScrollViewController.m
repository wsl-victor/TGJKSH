...
-(void)InitUI
{
    ...
    //设置总章节数
    [E_ReaderDataSource shareInstance].totalChapter = self.detailListARR.count;
    [E_ReaderDataSource shareInstance].allListDataArr = self.detailListARR;
...
-(void)requestNetWorkScrollWeb
{
    ...
    NSLog(@"==id==%@===11",self.detailDict[@"id"]);
    
    [AllReqestAPIManager getBookDetailClass:dict success:^(NSDictionary *json) {
        
        NSArray *tngouArr= json[@"list"];
        NSLog(@"===%ld===",tngouArr.count);
        [self.detailListARR addObjectsFromArray:tngouArr];
        [self InitUI];
